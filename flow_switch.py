...
class flowSw:
    ...
    async def _run(self, sampleInterval):
        _flowSw = False
        while True:                         ### Modification try to operate more reliable...
            if self.pumpCmd == 'ON':
                await asyncio.sleep_ms(sampleInterval) # Delay time to check
                _flowSw = self.flowSw.value() ###
                if _flowSw and self.pumpCmd == 'ON': ### Removed ++ True on _flowSw
                    self.err = 0
                else:
                ...
                await asyncio.sleep_ms(sampleInterval) # Delay time to check
                #_flowSw2nd = self.flowSw.value()
                _notFlowSw = not (_flowSw1st + self.flowSw.value()) ### new 
                if _notFlowSw and self.pumpCmd == 'OFF': # Pump is still OFF
                    self.err = 0
